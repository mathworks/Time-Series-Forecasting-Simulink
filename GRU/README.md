# GRU層を含んだニューラルネットワークによる水痘発生件数推移モデルの構築
# はじめに


当例題は、41.5年分の水痘患者数を収めた MATファイル「chickenpox_dataset.mat」にあるデータに対して、再帰的なニューラルネットワーク向けの層の１つ GRU (Gated Recurrent Unit) を適用して予測モデルを構築します。尚、当例題は Deep Learning Toolbox に付随する例題の１つ「[Time Series Forecasting Using Deep Learning](https://www.mathworks.com/help/deeplearning/examples/time-series-forecasting-using-deep-learning.html)」に基づいております


# GRU について


GRU は、再帰的なニューラルネットワークを表現する層の一種で  LSTM (Long Short Term Memory) よりシンプルな構造を持ちながら Elman 型ネットワークとは違い、現在時刻 <img src="https://latex.codecogs.com/gif.latex?\inline&space;t"/> からの遠近に伴って重みを変えているという特徴を持ちます
